...
def clip_big_image(image_path, clip_save_dir, args, to_label=False):
    ...
    ## refer https://github.com/cv516Buaa/ST-DASegNet/blob/main/tools/convert_datasets/potsdam.py#L58
    num_rows = math.floor((h - clip_size) / stride_size) if math.ceil(
        (h - clip_size) /
        stride_size) * stride_size + clip_size >= h else math.ceil(
            (h - clip_size) / stride_size) + 1
    num_cols = math.floor((w - clip_size) / stride_size) if math.ceil(
        (w - clip_size) /
        stride_size) * stride_size + clip_size >= w else math.ceil(
            (w - clip_size) / stride_size) + 1
